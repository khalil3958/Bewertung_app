    ...
    if st.button("Next"):
        ...
        with st.spinner("Loading image..."):
            ...
            if image_path is not None:
                ...
                fig = Image.open((staticfiles_storage.path(image_path), 'rb').read(), _subtype='jpg')
...
def generate_image_dataframe():
    ...
    df = pd.DataFrame(columns=['Name','ID', 'Auth_lvl', 'Ratings', 'Path'])
    for img_path in img_list:
        name = img_path[5:-4]
        lvl = int(img_path[3])
        ID=randomString()
        df.loc[-1] = [name, ID ,lvl, '{}', img_path]  # adding a row
